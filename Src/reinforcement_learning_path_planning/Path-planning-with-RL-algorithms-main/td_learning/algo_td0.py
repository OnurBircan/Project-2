...
class TemporalDifference:
    ...
    def check_state_exist(self, state):
        ...
        if state not in self.v_table.index:
            ...
            # new_row = pd.DataFrame([[0] * len(self.actions)], columns=self.v_table.columns, index=[state])
            new_row = pd.DataFrame([[0]], columns=self.v_table.columns, index=[state])

            self.v_table = pd.concat([self.v_table, new_row])
